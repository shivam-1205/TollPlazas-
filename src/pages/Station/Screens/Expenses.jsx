...
import { PlusCircle, Info, Currency } from 'lucide-react';
...
export default function Expenses() {
    ...
    return (
        <div className="font-Poppins p-6 bg-gray-50 min-h-screen">
            {show && <AddExpenses setShow={setShow} />}
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-3xl font-semibold text-gray-800 flex items-center gap-2">
                    <Currency className="text-indigo-600" /> Expenses
                </h2>
                <button
                ...
                {expense?.length > 0 ? (
                    <table className="min-w-full table-auto text-sm text-gray-700">
                        <thead className="bg-gray-100 text-xs uppercase text-gray-600">
                            <tr>
                                {Object.keys(expense[0]).map((key, idx) => (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                        ...
                        <p className="italic text-lg">No Expenses data available</p>
                    </div>
                )}
            </div>
        </div>
